...
#' @param from The initial abundance unit. Options: "n" (number of individuals), "nsub" (number of individuals in a subsample), "L" (number of individuals per litre), "m3" (number of individuals per cubic metre), "m2" (number of individuals per square metre).
#' @param to The desired abundance unit. Options: "L" (number of individuals per litre), "m3" (number of individuals per cubic metre), "m2" (number of individuals per square metre).
...
#' @param 
#' @param V name of the column giving volume of melted water in mL
#' @param sec.start name of the column in \code{data} specifying the section start for the ice core in cm.
#' @param sec.end name of the column in \code{data} specifying the section end for the ice core in cm.
#' @param cf concentration factor connected to the volume difference when transforming water to ice (melted water volume/ice volume)
...
convert_abundance <- function(data, ab_col, ab_from = NULL, ab_to = NULL, filtered = NULL, type = "icecore", cf = 1, vol_col = NULL, start_col = "from", end_col = "to", unit_col = "unit") {
  ...
  if(filtered) {
    ...
    if(ab_from == "L" & ab_to == "m2") {
      ...
      stop("Nothing has been implemented yet")
      }
